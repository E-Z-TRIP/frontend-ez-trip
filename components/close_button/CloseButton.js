...
export default function CloseButton({ style, onPress, iconScale, iconColor, activeOpacity }) {
  return (
    <TouchableOpacity style={style} onPress={onPress} activeOpacity={activeOpacity}>
      <CrossIcon scale={iconScale} color={iconColor} />
    </TouchableOpacity>
  );
}

function CrossIcon({ scale = 1, color }) {
